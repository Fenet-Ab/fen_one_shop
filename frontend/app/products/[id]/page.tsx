...
export default function ProductDetail() {
    ...
    const fetchProductData = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/material/${params.id}`);
            if (!response.ok) throw new Error('Product not found');
            const data = await response.json();
            setProduct(data);

            // Fetch rating stats
            const statsRes = await fetch(`http://localhost:5000/api/rating/material/${params.id}/stats`);
            if (statsRes.ok) {
            ...
            if (token) {
                const myRatingRes = await fetch(`http://localhost:5000/api/rating/my/${params.id}`, {
    ...
    const handleSubmitReview = async () => {
        ...
        setIsRating(true);
        try {
            const response = await fetch(`http://localhost:5000/api/rating/${params.id}`, {
                method: 'POST',
                headers: {
