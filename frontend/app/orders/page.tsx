...
import { Package, ChevronRight, Calendar, CreditCard, Clock, CheckCircle2, Loader2, ArrowRight, Trash2, X as XIcon } from "lucide-react";
    ...
    useEffect(() => {
        ...
        if (verifyId && !verifying) {
            const verifyPayment = async () => {
                ...
                try {
                    const url = txRef
                        ? `http://localhost:5000/api/payment/verify/${verifyId}?tx_ref=${txRef}`
                        : `http://localhost:5000/api/payment/verify/${verifyId}`;
        ...
        return (
            <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
                <div className="bg-gray-50 p-12 rounded-[3rem] text-center max-w-md w-full border border-gray-100">
                    ...
                    <Link href="/login" className="block w-full bg-[#1A1A1A] text-white py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-[#D4AF37] transition-all">
                        Login
                    </Link>
                </div>
            </div>
                                        ...
                                        <div className="space-y-6 flex-1">
                                            ...
                                            <div className="flex -space-x-4 overflow-hidden">
                                                {order.items.slice(0, 4).map((item, idx) => (
                                                    <div key={idx} className="w-16 h-16 rounded-2xl bg-[#FBFBFB] border-4 border-white overflow-hidden p-2">
